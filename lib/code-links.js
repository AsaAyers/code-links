...
module.exports = {
    ...
    activate() {
        ...
        this.subs.add(atom.config.observe('code-links.modifierKey', (modifier) => {
            ...
            document.body.classList.remove('code-links-meta');
        ...
        if (!atom.packages.isPackageActive('coffee-links')) {
            const detail = `

            code-links is no longer being maintained. You can get
            the same functionality using 'hyperclick' with
            'js-hyperclick'.

            `.split('\n').map(s => s.trim()).join('\n')

            atom.notifications.addWarning('code-links', {
                detail,
                dismissable: true
            });
        }
    ...
    setupClickHandler(editor) {
        ...
        this.subs.add(links, 'click', '.code-links .region', (e) => {
            ...
            } else if ((modifier === 'ctrl' && e.ctrlKey) || (modifier === 'meta' && e.metaKey)) {
